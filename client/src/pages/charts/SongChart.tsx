/**
 * SongChart - Song/Track chart page (K-STAR TOP 100)
 */
...
export default function SongChart() {
  ...
  const { data: weeks, loading: weeksLoading } = useAvailableWeeks("songs");
  ...
  const goToPrevious = useCallback(() => {
    if (hasPrevious && weeks) {
      ...
      setLocation(`/charts/songs/${prevWeek}`);
    }
  ...
  const goToNext = useCallback(() => {
    if (hasNext && weeks) {
      ...
      setLocation(`/charts/songs/${nextWeek}`);
    }
  }, [hasNext, weeks, currentIndex, setLocation]);

  const goToWeek = useCallback(
    (week: string) => {
      setLocation(`/charts/songs/${week}`);
    },
    [setLocation]
  );

  // Load chart data for current week
  const { data: chartData, loading: chartLoading } = useSongsChart(currentWeek || undefined);
